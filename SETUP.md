...
**21 commands** — `/start`, `/sync`, `/wrap-up`, `/safe-clear`, `/audit`, `/onboard`, `/review`, `/retro`, `/launch`, `/report`, and more. Type them in Claude Code and the system handles the rest.
...
| `/safe-clear` | Resume after losing context or between unrelated tasks |
...
1. **Run `/safe-clear` between unrelated tasks.** Context pollution is the #1 quality killer.
